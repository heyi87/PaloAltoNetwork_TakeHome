...
Model: TweedieRegressor (GLM with log link).

Designed for zero-inflated, highly skewed targets (many zeros, few high spenders).

Predicts expected spend directly in dollars.

Preprocessing: numeric features are imputed (median) and standardized; categorical features are imputed and one-hot encoded via a ColumnTransformer inside a Pipeline.
